...
from lecode.agent.tools import Tool, ToolRegistry
from lecode.agent.tools import ToolResult as ToolExecResult
...
async def test_child_can_read_shared_facts_with_project_context(tmp_path, monkeypatch):
    ...
    class FactReader(Tool):
        async def run(self, args, ctx):
            ...
            return ToolExecResult(ctx.recall_context.recall({"fact_id": fact.id}))
