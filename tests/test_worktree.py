...
async def shell_validation(cmd, cwd):
    """An approved test caller that intentionally runs validation commands."""
    result = await run_proc(["sh", "-lc", cmd], cwd=cwd, timeout=30)
...
# -- worker worktrees: create/attach/inspect/integrate/cleanup -----------------------
...
async def test_integrate_happy_path(tmp_path):
    repo = await make_repo(tmp_path / "repo")
    manager = WorktreeManager(repo)
    base = await git(repo, "rev-parse", "HEAD")
    info = await manager.create_worker(
        "worker", base_commit=base, dest_path=repo, dest_branch="main"
    )
    (info.path / "feature.txt").write_text("feature\n", encoding="utf-8")
    await commit_all(info.path, "add feature")

    result = await manager.integrate(
        "worker", validation=["test -f feature.txt"], validation_runner=shell_validation
    )
    assert result.status == "integrated"
    assert result.conflicts == []
    assert (repo / "feature.txt").read_text() == "feature\n"
    sidecar = manager.read_sidecar("worker")
    assert sidecar["integrated_at"]
    assert sidecar["integrated_head"] == await git(repo, "rev-parse", "HEAD")


async def test_integrate_requires_validation_unless_explicitly_allowed(tmp_path):
    repo = await make_repo(tmp_path / "repo")
    manager = WorktreeManager(repo)
    base = await git(repo, "rev-parse", "HEAD")
    info = await manager.create_worker(
    ...
    denied = await manager.integrate("worker", validation=[])
    assert denied.status == "blocked"
    assert denied.detail == "validation required"
    allowed = await manager.integrate("worker", validation=[], allow_unvalidated=True)
    assert allowed.status == "integrated"


async def test_integrate_refuses_worker_not_at_reviewed_head(tmp_path):
    repo = await make_repo(tmp_path / "repo")
    manager = WorktreeManager(repo)
    base = await git(repo, "rev-parse", "HEAD")
    info = await manager.create_worker(
    ...
    with pytest.raises(WorktreeError, match="differs from reviewed"):
...
async def test_integrate_rejects_destination_changed_during_validation(tmp_path):
    repo = await make_repo(tmp_path / "repo")
    manager = WorktreeManager(repo)
    base = await git(repo, "rev-parse", "HEAD")
    info = await manager.create_worker(
    ...
    async def change_destination(_cmd, _cwd):
        (repo / "moved.txt").write_text("moved\n", encoding="utf-8")
        await commit_all(repo, "move destination")
        return 0, ""

    result = await manager.integrate(
        "worker", validation=["approved"], validation_runner=change_destination
    )
    assert result.status == "paused"
    assert result.detail == "destination changed during validation"
    assert not (repo / "feature.txt").exists()


async def test_integrate_without_sidecar_pauses(tmp_path):
    repo = await make_repo(tmp_path / "repo")
    manager = WorktreeManager(repo)
    await manager.create("plain")
    result = await manager.integrate("plain", validation=[], allow_unvalidated=True)
    assert result.status == "paused"
    assert result.detail == "no destination recorded"


async def test_integrate_validation_failure_blocks_merge(tmp_path):
    repo = await make_repo(tmp_path / "repo")
    manager = WorktreeManager(repo)
    base = await git(repo, "rev-parse", "HEAD")
    info = await manager.create_worker(
    ...
    result = await manager.integrate(
        "worker", validation=["echo boom >&2; exit 1"], validation_runner=shell_validation
    )
    assert result.status == "validation_failed"
    assert "echo boom" in result.detail
    assert "boom" in result.validation_output
    assert not (repo / "feature.txt").exists()
    assert await git(repo, "rev-parse", "HEAD") == base
    assert manager.read_sidecar("worker")["integrated_at"] is None


async def test_integrate_pauses_and_blocks(tmp_path):
    repo = await make_repo(tmp_path / "repo")
    manager = WorktreeManager(repo)
    base = await git(repo, "rev-parse", "HEAD")
    info = await manager.create_worker(
    ...
    (info.path / "dirty.txt").write_text("x\n", encoding="utf-8")
    result = await manager.integrate("worker", validation=[], allow_unvalidated=True)
    assert result.status == "blocked"
    assert result.detail == "uncommitted changes"
    (info.path / "dirty.txt").unlink()

    (repo / "uncommitted.txt").write_text("x\n", encoding="utf-8")
    result = await manager.integrate("worker", validation=[], allow_unvalidated=True)
    assert result.status == "paused"
    assert result.detail == "destination has uncommitted changes"
    (repo / "uncommitted.txt").unlink()

    await git(repo, "checkout", "-b", "side")
    result = await manager.integrate("worker", validation=[], allow_unvalidated=True)
    assert result.status == "paused"
    assert result.detail == "destination is on 'side', expected 'main'"


async def test_integrate_replays_destination_before_validation(tmp_path):
    repo = await make_repo(tmp_path / "repo")
    manager = WorktreeManager(repo)
    base = await git(repo, "rev-parse", "HEAD")
    info = await manager.create_worker(
    ...
    (repo / "later.txt").write_text("later\n", encoding="utf-8")
    await commit_all(repo, "later")
    dest_commit = await git(repo, "rev-parse", "HEAD")

    result = await manager.integrate(
        "worker", validation=["test -f later.txt"], validation_runner=shell_validation
    )
    assert result.status == "integrated"
    ancestor = await run_proc(
        ["git", "merge-base", "--is-ancestor", dest_commit, "lecode/worker"],
        cwd=repo,
        timeout=30,
    )
    assert ancestor.exit_code == 0


async def test_integrate_conflict_leaves_merge_in_progress(tmp_path):
    repo = await make_repo(tmp_path / "repo")
    manager = WorktreeManager(repo)
    base = await git(repo, "rev-parse", "HEAD")
    info = await manager.create_worker(
    ...
    assert result.conflicts == ["file.txt"]
    assert (await manager.inspect("worker")).merge_in_progress is True
    assert manager.read_sidecar("worker")["integrated_at"] is None


async def test_cleanup_worker_guards(tmp_path):
    repo = await make_repo(tmp_path / "repo")
    manager = WorktreeManager(repo)
    base = await git(repo, "rev-parse", "HEAD")
    info = await manager.create_worker(
    ...
    with pytest.raises(WorktreeError, match="not integrated"):
        await manager.cleanup_worker("worker")

    (info.path / "feature.txt").write_text("feature\n", encoding="utf-8")
    await commit_all(info.path, "add feature")
    assert (
        await manager.integrate("worker", validation=[], allow_unvalidated=True)
    ).status == "integrated"
    (info.path / "dirty.txt").write_text("x\n", encoding="utf-8")
    with pytest.raises(WorktreeError, match="uncommitted changes"):
        await manager.cleanup_worker("worker")

    other = await manager.create_worker(
        "other", base_commit=base, dest_path=repo, dest_branch="main"
    )
    removed = await manager.cleanup_worker("other", discard=True)
    assert removed == other
    assert not other.path.exists()
    branch = await run_proc(
        ["git", "show-ref", "--verify", "refs/heads/lecode/other"], cwd=repo, timeout=30
    )
    assert branch.exit_code != 0

    await manager.cleanup_worker("worker", discard=True)
    assert not info.path.exists()


async def test_cleanup_worker_refuses_commit_after_integration(tmp_path):
    repo = await make_repo(tmp_path / "repo")
    manager = WorktreeManager(repo)
    base = await git(repo, "rev-parse", "HEAD")
    info = await manager.create_worker(
    ...
    (info.path / "after.txt").write_text("after\n", encoding="utf-8")
    await commit_all(info.path, "post-integration commit")

    with pytest.raises(WorktreeError, match="not fully integrated"):
        await manager.cleanup_worker("worker")
    await manager.cleanup_worker("worker", discard=True)


async def test_concurrent_integrations_are_serialized(tmp_path):
    repo = await make_repo(tmp_path / "repo")
    manager = WorktreeManager(repo)
    base = await git(repo, "rev-parse", "HEAD")
    first = await manager.create_worker(
        "first", base_commit=base, dest_path=repo, dest_branch="main"
    )
    second = await manager.create_worker(
        "second", base_commit=base, dest_path=repo, dest_branch="main"
    )
    (first.path / "first.txt").write_text("first\n", encoding="utf-8")
    await commit_all(first.path, "first")
    (second.path / "second.txt").write_text("second\n", encoding="utf-8")
    await commit_all(second.path, "second")
    first_started = asyncio.Event()
    release_first = asyncio.Event()
    second_started = asyncio.Event()

    async def validate(cmd, _cwd):
        if cmd == "first":
            first_started.set()
    ...
    first_task = asyncio.create_task(
        manager.integrate("first", validation=["first"], validation_runner=validate)
    )
    await first_started.wait()
    second_task = asyncio.create_task(
        manager.integrate("second", validation=["second"], validation_runner=validate)
    )
    await asyncio.sleep(0)
    assert not second_started.is_set()
    release_first.set()
    assert (await first_task).status == "integrated"
    assert (await second_task).status == "integrated"
    assert (repo / "first.txt").is_file()
    assert (repo / "second.txt").is_file()


async def test_paused_integrate_is_non_destructive(tmp_path):
    repo = await make_repo(tmp_path / "repo")
    manager = WorktreeManager(repo)
    base = await git(repo, "rev-parse", "HEAD")
    info = await manager.create_worker(
    ...
    assert await git(repo, "status", "--porcelain") == "?? uncommitted.txt"
    assert await git(repo, "rev-parse", "HEAD") == base
